...
import {
  useFavorites,
  useF1Circuits as useApi,
} from '@sudobility/heavymath_indexer_client';
const mockUseApi = vi.mocked(useApi);
const mockUseFavorites = vi.mocked(useFavorites);
const mockData = [
  { id: 100, name: 'Circuit de Monaco' },
  { id: 101, name: 'Silverstone' },
];
const mockFavs = [
  {
    id: 1,
...
describe('useF1Circuits', () => {
  ...
  it('should return circuits with favorited flag', () => {
    const { result } = renderHook(() => useF1Circuits(ic, wa), {
      wrapper: cw(),
    });
    expect(result.current.circuits).toHaveLength(2);
    expect(result.current.circuits[0].favorited).toBe(true);
    expect(result.current.circuits[1].favorited).toBe(false);
  });
  it('should call addFavorite', async () => {
    ...
    await act(async () => {
      await result.current.setFavorited(100, true);
    });
  ...
  it('should pass correct filters', () => {
    renderHook(() => useF1Circuits(ic, wa), { wrapper: cw() });
  ...
  it('should return empty when undefined', () => {
    ...
    expect(result.current.circuits).toHaveLength(0);
  });
});
