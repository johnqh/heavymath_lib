...
vi.mock('@sudobility/sports_api_client', () => ({ useF1Teams: vi.fn() }));
vi.mock('@sudobility/heavymath_indexer_client', () => ({ useFavorites: vi.fn() }));
import { useF1Teams as useApi } from '@sudobility/sports_api_client';
...
const mockData = [{ id: 10, name: 'Red Bull Racing' }, { id: 20, name: 'Mercedes' }];
const mockFavs = [{ id: 1, itemId: '10', category: 'sports', subcategory: 'f1', type: 'team' }];
...
describe('useF1Teams', () => {
  ...
  it('should return teams with favorited flag', () => { const { result } = renderHook(() => useF1Teams(ic, wa), { wrapper: cw() }); expect(result.current.teams).toHaveLength(2); expect(result.current.teams[0].favorited).toBe(true); expect(result.current.teams[1].favorited).toBe(false); });
  it('should call addFavorite', async () => { const m = vi.fn().mockResolvedValue({}); mockUseFavorites.mockReturnValue({ favorites: [], isLoading: false, isError: false, error: null, addFavorite: { mutateAsync: m, isPending: false }, removeFavorite: { mutateAsync: vi.fn(), isPending: false }, refresh: vi.fn() } as any); const { result } = renderHook(() => useF1Teams(ic, wa), { wrapper: cw() }); await act(async () => { await result.current.setFavorited(10, true); }); expect(m).toHaveBeenCalledWith({ category: 'sports', subcategory: 'f1', type: 'team', id: '10' }); });
  it('should pass correct filters', () => { renderHook(() => useF1Teams(ic, wa), { wrapper: cw() }); expect(mockUseFavorites).toHaveBeenCalledWith(ic, wa, { category: 'sports', subcategory: 'f1', type: 'team' }); });
  it('should return empty when undefined', () => { mockUseApi.mockReturnValue({ data: undefined, isLoading: false, isError: false, error: null } as any); const { result } = renderHook(() => useF1Teams(ic, wa), { wrapper: cw() }); expect(result.current.teams).toHaveLength(0); });
});
