...
import { useHandballLeagues } from '../useHandballLeagues';
vi.mock('@sudobility/heavymath_indexer_client', () => ({
  useFavorites: vi.fn(),
  useHandballLeagues: vi.fn(),
}));
import {
  useFavorites,
  useHandballLeagues as useApi,
} from '@sudobility/heavymath_indexer_client';
const mockUseApi = vi.mocked(useApi);
const mockUseFavorites = vi.mocked(useFavorites);
const mockData = [
  { id: 1, name: 'EHF Champions League' },
...
describe('useHandballLeagues', () => {
  ...
  it('should return leagues with favorited flag', () => {
    const { result } = renderHook(() => useHandballLeagues(ic, wa), {
      wrapper: cw(),
    });
  ...
  it('should pass correct filters', () => {
    renderHook(() => useHandballLeagues(ic, wa), { wrapper: cw() });
