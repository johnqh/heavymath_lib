...
vi.mock('@sudobility/sports_api_client', () => ({ useMmaFighters: vi.fn() }));
vi.mock('@sudobility/heavymath_indexer_client', () => ({ useFavorites: vi.fn() }));
import { useMmaFighters as useApi } from '@sudobility/sports_api_client';
...
const mockData = [{ id: 10, name: 'Fighter A' }, { id: 20, name: 'Fighter B' }];
const mockFavs = [{ id: 1, itemId: '10', category: 'sports', subcategory: 'mma', type: 'fighter' }];
...
describe('useMmaFighters', () => {
  ...
  it('should return fighters with favorited flag', () => { const { result } = renderHook(() => useMmaFighters(ic, wa), { wrapper: cw() }); expect(result.current.fighters).toHaveLength(2); expect(result.current.fighters[0].favorited).toBe(true); expect(result.current.fighters[1].favorited).toBe(false); });
  it('should call addFavorite', async () => { const m = vi.fn().mockResolvedValue({}); mockUseFavorites.mockReturnValue({ favorites: [], isLoading: false, isError: false, error: null, addFavorite: { mutateAsync: m, isPending: false }, removeFavorite: { mutateAsync: vi.fn(), isPending: false }, refresh: vi.fn() } as any); const { result } = renderHook(() => useMmaFighters(ic, wa), { wrapper: cw() }); await act(async () => { await result.current.setFavorited(10, true); }); expect(m).toHaveBeenCalledWith({ category: 'sports', subcategory: 'mma', type: 'fighter', id: '10' }); });
  it('should pass correct filters', () => { renderHook(() => useMmaFighters(ic, wa), { wrapper: cw() }); expect(mockUseFavorites).toHaveBeenCalledWith(ic, wa, { category: 'sports', subcategory: 'mma', type: 'fighter' }); });
  it('should return empty when undefined', () => { mockUseApi.mockReturnValue({ data: undefined, isLoading: false, isError: false, error: null } as any); const { result } = renderHook(() => useMmaFighters(ic, wa), { wrapper: cw() }); expect(result.current.fighters).toHaveLength(0); });
});
