...
import { useMmaFights } from '../useMmaFights';
...
describe('useMmaFights', () => {
  ...
  it('should return fights with favorited flag', () => {
    const { result } = renderHook(() => useMmaFights(ic, wa), {
      wrapper: cw(),
    });
    expect(result.current.fights).toHaveLength(2);
    expect(result.current.fights[0].favorited).toBe(true);
    expect(result.current.fights[1].favorited).toBe(false);
  });
  it('should call addFavorite', async () => {
    ...
    await act(async () => {
      await result.current.setFavorited(301, true);
    });
  ...
  it('should pass correct filters', () => {
    renderHook(() => useMmaFights(ic, wa), { wrapper: cw() });
  ...
  it('should return empty when undefined', () => {
    ...
    expect(result.current.fights).toHaveLength(0);
  });
});
