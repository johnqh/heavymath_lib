...
vi.mock('@sudobility/sports_api_client', () => ({ useNflGames: vi.fn() }));
vi.mock('@sudobility/heavymath_indexer_client', () => ({ useFavorites: vi.fn() }));

import { useNflGames as useNflGamesApi } from '@sudobility/sports_api_client';
...
const mockFavorites = [{ id: 1, itemId: '101', category: 'sports', subcategory: 'nfl', type: 'game' }];
...
describe('useNflGames', () => {
  ...
  it('should return games with favorited flag', () => {
    const { result } = renderHook(() => useNflGames(mockIndexerClient, mockWalletAddress), { wrapper: createWrapper() });
  ...
  it('should call addFavorite', async () => {
    ...
    expect(mock).toHaveBeenCalledWith({ category: 'sports', subcategory: 'nfl', type: 'game', id: '101' });
  });

  it('should pass correct filters to useFavorites', () => {
    renderHook(() => useNflGames(mockIndexerClient, mockWalletAddress), { wrapper: createWrapper() });
    expect(mockUseFavorites).toHaveBeenCalledWith(mockIndexerClient, mockWalletAddress, { category: 'sports', subcategory: 'nfl', type: 'game' });
  });
