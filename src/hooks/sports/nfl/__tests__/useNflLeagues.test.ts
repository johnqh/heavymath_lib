...
vi.mock('@sudobility/sports_api_client', () => ({ useNflLeagues: vi.fn() }));
vi.mock('@sudobility/heavymath_indexer_client', () => ({ useFavorites: vi.fn() }));

import { useNflLeagues as useNflLeaguesApi } from '@sudobility/sports_api_client';
...
const mockData = [{ league: { id: 1, name: 'NFL' }, country: { name: 'USA' }, seasons: [] }, { league: { id: 2, name: 'CFL' }, country: { name: 'Canada' }, seasons: [] }];
const mockFavorites = [{ id: 1, itemId: '1', category: 'sports', subcategory: 'nfl', type: 'league' }];
...
describe('useNflLeagues', () => {
  ...
  it('should return leagues with favorited flag', () => {
    const { result } = renderHook(() => useNflLeagues(mockIndexerClient, mockWalletAddress), { wrapper: createWrapper() });
  ...
  it('should call addFavorite', async () => {
    ...
    expect(mock).toHaveBeenCalledWith({ category: 'sports', subcategory: 'nfl', type: 'league', id: '1' });
  });
  ...
  it('should pass correct filters to useFavorites', () => {
    renderHook(() => useNflLeagues(mockIndexerClient, mockWalletAddress), { wrapper: createWrapper() });
    expect(mockUseFavorites).toHaveBeenCalledWith(mockIndexerClient, mockWalletAddress, { category: 'sports', subcategory: 'nfl', type: 'league' });
  });
