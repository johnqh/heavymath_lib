...
const mockFavorites = [
  { id: 1, itemId: '10', category: 'sports', subcategory: 'nfl', type: 'team' },
];
...
describe('useNflTeams', () => {
  ...
  it('should return teams with favorited flag', () => {
    const { result } = renderHook(
      () => useNflTeams(mockIndexerClient, mockWalletAddress),
  ...
  it('should pass correct filters to useFavorites', () => {
    renderHook(() => useNflTeams(mockIndexerClient, mockWalletAddress), {
      wrapper: createWrapper(),
    });
    expect(mockUseFavorites).toHaveBeenCalledWith(
      mockIndexerClient,
      mockWalletAddress,
      { category: 'sports', subcategory: 'nfl', type: 'team' }
    );
  });
