...
vi.mock('@sudobility/sports_api_client', () => ({ useBaseballGames: vi.fn() }));
vi.mock('@sudobility/heavymath_indexer_client', () => ({ useFavorites: vi.fn() }));

import { useBaseballGames as useBaseballGamesApi } from '@sudobility/sports_api_client';
...
const mockData = [{ id: 101, date: '2024-01-15' }, { id: 102, date: '2024-01-16' }];
const mockFavorites = [{ id: 1, itemId: '101', category: 'sports', subcategory: 'baseball', type: 'game' }];
...
describe('useBaseballGames', () => {
  ...
  it('should return items with favorited flag', () => {
    const { result } = renderHook(() => useBaseballGames(mockIndexerClient, mockWalletAddress), { wrapper: createWrapper() });
  ...
  it('should call addFavorite', async () => {
    ...
    expect(mock).toHaveBeenCalledWith({ category: 'sports', subcategory: 'baseball', type: 'game', id: '101' });
  });
  ...
  it('should pass correct filters to useFavorites', () => {
    renderHook(() => useBaseballGames(mockIndexerClient, mockWalletAddress), { wrapper: createWrapper() });
    expect(mockUseFavorites).toHaveBeenCalledWith(mockIndexerClient, mockWalletAddress, { category: 'sports', subcategory: 'baseball', type: 'game' });
  });
