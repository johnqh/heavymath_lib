...
import { useBaseballTeams } from '../useBaseballTeams';

vi.mock('@sudobility/sports_api_client', () => ({ useBaseballTeams: vi.fn() }));
vi.mock('@sudobility/heavymath_indexer_client', () => ({ useFavorites: vi.fn() }));

import { useBaseballTeams as useBaseballTeamsApi } from '@sudobility/sports_api_client';
import { useFavorites } from '@sudobility/heavymath_indexer_client';

const mockUseApi = vi.mocked(useBaseballTeamsApi);
const mockUseFavorites = vi.mocked(useFavorites);
const mockData = [{ id: 10, name: 'Team A' }, { id: 20, name: 'Team B' }];
const mockFavorites = [{ id: 1, itemId: '10', category: 'sports', subcategory: 'baseball', type: 'team' }];
...
describe('useBaseballTeams', () => {
  ...
  it('should return items with favorited flag', () => {
    const { result } = renderHook(() => useBaseballTeams(mockIndexerClient, mockWalletAddress), { wrapper: createWrapper() });
  ...
  it('should call addFavorite', async () => {
    ...
    expect(mock).toHaveBeenCalledWith({ category: 'sports', subcategory: 'baseball', type: 'team', id: '10' });
  });

  it('should call removeFavorite', async () => {
    const mock = vi.fn().mockResolvedValue({});
    mockUseFavorites.mockReturnValue({ favorites: mockFavorites, isLoading: false, isError: false, error: null, addFavorite: { mutateAsync: vi.fn(), isPending: false }, removeFavorite: { mutateAsync: mock, isPending: false }, refresh: vi.fn() } as any);
    const { result } = renderHook(() => useBaseballTeams(mockIndexerClient, mockWalletAddress), { wrapper: createWrapper() });
    await act(async () => { await result.current.setFavorited(10, false); });
    expect(mock).toHaveBeenCalledWith(1);
  });

  it('should pass correct filters to useFavorites', () => {
    renderHook(() => useBaseballTeams(mockIndexerClient, mockWalletAddress), { wrapper: createWrapper() });
    expect(mockUseFavorites).toHaveBeenCalledWith(mockIndexerClient, mockWalletAddress, { category: 'sports', subcategory: 'baseball', type: 'team' });
  });
