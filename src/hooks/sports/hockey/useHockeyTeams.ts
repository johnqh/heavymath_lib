...
import {
  ...
  useFavoriteCounts,
  useFavorites,
  useHockeyTeams as useHockeyTeamsProxy,
...
export interface HockeyTeamWithFavorite extends HockeyTeamResponse {
  ...
  favorited: boolean;
  /** Total number of users who have favorited this team */
...
export function useHockeyTeams(
  ...
  const teamsQuery = useHockeyTeamsProxy(
    indexerClient,
  ...
  const teamIds = useMemo(() => {
    const response = (teamsQuery.data?.response ?? []) as HockeyTeamResponse[];
    return response.map(t => String(t.id));
