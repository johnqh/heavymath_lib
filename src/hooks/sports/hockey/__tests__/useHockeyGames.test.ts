...
vi.mock('@sudobility/sports_api_client', () => ({ useHockeyGames: vi.fn() }));
vi.mock('@sudobility/heavymath_indexer_client', () => ({ useFavorites: vi.fn() }));
import { useHockeyGames as useApi } from '@sudobility/sports_api_client';
...
const mockFavs = [{ id: 1, itemId: '101', category: 'sports', subcategory: 'hockey', type: 'game' }];
...
describe('useHockeyGames', () => {
  ...
  it('should return games with favorited flag', () => { const { result } = renderHook(() => useHockeyGames(ic, wa), { wrapper: cw() }); expect(result.current.games).toHaveLength(2); expect(result.current.games[0].favorited).toBe(true); expect(result.current.games[1].favorited).toBe(false); });
  it('should call addFavorite', async () => { const m = vi.fn().mockResolvedValue({}); mockUseFavorites.mockReturnValue({ favorites: [], isLoading: false, isError: false, error: null, addFavorite: { mutateAsync: m, isPending: false }, removeFavorite: { mutateAsync: vi.fn(), isPending: false }, refresh: vi.fn() } as any); const { result } = renderHook(() => useHockeyGames(ic, wa), { wrapper: cw() }); await act(async () => { await result.current.setFavorited(101, true); }); expect(m).toHaveBeenCalledWith({ category: 'sports', subcategory: 'hockey', type: 'game', id: '101' }); });
  it('should pass correct filters', () => { renderHook(() => useHockeyGames(ic, wa), { wrapper: cw() }); expect(mockUseFavorites).toHaveBeenCalledWith(ic, wa, { category: 'sports', subcategory: 'hockey', type: 'game' }); });
  it('should return empty when undefined', () => { mockUseApi.mockReturnValue({ data: undefined, isLoading: false, isError: false, error: null } as any); const { result } = renderHook(() => useHockeyGames(ic, wa), { wrapper: cw() }); expect(result.current.games).toHaveLength(0); });
});
