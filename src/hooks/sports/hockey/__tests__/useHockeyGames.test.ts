...
import { useHockeyGames } from '../useHockeyGames';
vi.mock('@sudobility/heavymath_indexer_client', () => ({
  useFavorites: vi.fn(),
  useHockeyGames: vi.fn(),
}));
import {
  useFavorites,
  useHockeyGames as useApi,
...
describe('useHockeyGames', () => {
  ...
  it('should return games with favorited flag', () => {
    const { result } = renderHook(() => useHockeyGames(ic, wa), {
      wrapper: cw(),
    });
  ...
  it('should pass correct filters', () => {
    renderHook(() => useHockeyGames(ic, wa), { wrapper: cw() });
